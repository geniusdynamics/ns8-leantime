...
reponame="leantime"
leantime_version="3.1.4"
# Create a new empty container image
container=$(buildah from scratch)

# Reuse existing nodebuilder-leantime container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-leantime; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-leantime -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/mariadb:10.11.5 docker.io/leantime/leantime:${leantime_version}" \
    "${container}"
# Commit the image
